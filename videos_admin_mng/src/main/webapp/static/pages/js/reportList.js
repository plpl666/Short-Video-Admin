var forbidVideo = function(videoId) {
    ...
    // var flag;
    // if (status === 1) {
     //    flag = window.confirm("是否禁播?");
    // } else if (status === 2) {
     //    flag = window.confirm("是否复播?");
    // }
	// if (!flag) {
	// 	return;
	// }
	...
	$.ajax({
    	url: $("#hdnContextPath").val() + "/videos/forbidVideo?videoId=" + videoId,
    	type: "POST",
    	async: false,
...
var UsersReportsList = function () {
	...
	var handleUsersReportsList = function() {
        ...
        jqGrid.jqGrid({  
            caption: "被举报的视频列表",  
            url: hdnContextPath + "/videos/queryReportList",
            mtype: "post",  
                ...
                { name: '', index: '', width: 20, sortable: false, hidden: false,
                	formatter:function(cellvalue, options, rowObject) {
                        ...
                        if (rowObject.status === 1) {
                            button = "<button class='btn btn-outline blue-chambray' id='' onclick=forbidVideo(\"" + rowObject.dealVideoId + "\") style='padding: 1px 3px 1px 3px;'>禁止播放</button>";
                        } else if (rowObject.status === 2) {
                            button = "<button class='btn btn-outline blue-chambray' id='' onclick=forbidVideo(\"" + rowObject.dealVideoId + "\") style='padding: 1px 3px 1px 3px;'>恢复播放</button>";
                        }
			    		return button;
			    	}
			    }
